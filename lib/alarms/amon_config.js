...
function parseProbeGroupName(probeGroupName)
{
	...
	verpart = probeGroupName.substr(i + 2);
	if (verpart != '1') {
...
MantaAmonMetadata.prototype.addTemplate = function addTemplate(args)
{
	...
	pt = {};
	pt.pt_origin_label = args.originLabel;
	pt.pt_event = inp.event;
	pt.pt_scope = {};
	pt.pt_scope.ptsc_service = inp.scope.service;
	pt.pt_scope.ptsc_global = (inp.scope.global === true);
	...
	inp.checks.forEach(function (c) {
		...
		pt.pt_checks.push(cc);
	});

	pt.pt_ka = {};
	pt.pt_ka.ka_title = inp.ka.title;
	pt.pt_ka.ka_description = inp.ka.description;
	pt.pt_ka.ka_severity = inp.ka.severity;
	pt.pt_ka.ka_response = inp.ka.response;
	pt.pt_ka.ka_impact = inp.ka.impact;
	pt.pt_ka.ka_action = inp.ka.action;
	pt.pt_aliases = [];
 ...
 *
 * This function returns either an Error (on failure) or an object with several
 * properties.  These should not be used outside of amonUpdatePlanApply().
 *
 * XXX make a new class for this: AmonUpdatePlan
 *
 *     ngroupsmatched		number of probe groups that were deployed and
 *     				wanted
 *
 *     nprobesmatched		number of probes that were deployed and wanted
 *
 *     ngroupsignored		number of probe groups that were deployed,
 *     				unwanted, but kept (likely because we didn't
 *     				recognize them and figure they were
 *     				operator-created groups)
 *
 *     probesToRemove		list of uuids of probes to be removed
 *
 *     probeGroupsToRemove	list of uuids of probe groups to be removed
 *
 *     probeGroupsToAdd		list of probe group objects to add
 *
 *     probesToAdd		list of probes to be created.  Since the uuids
 *     				of these probes' groups cannot necessarily be
 *     				known ahead of time, these are linked to
 *     				corresponding probe groups by name.
 *
 *     warnings			list of Errors describing situations that are
 *     				not as expected, not necessarily bugs, and not
 *     				automatically correctable.  End users should be
 *     				notified about these issues.
 */
function amonUpdatePlanCreate(args)
{
	...
	wanted.eachProbeGroup(function iterWProbeGroup(wpg) {
		...
		if (dpg !== null) {
			...
			if (wpg.pg_user != dpg.pg_user ||
				 ...
				 * unexpected.
				 */
				rv.warnings.push(new VError('probe group ' +
				    'with name "%s" (deployed with uuid ' +
				    '"%s"): unexpected "user" or "contacts"',
				    pgname, dpg.pg_uuid));
			}
		} else {
			rv.probeGroupsToAdd.push(wpg);
		}
	...
	deployed.eachProbeGroup(function iterDProbeGroup(dpg) {
		...
		rv.probeGroupsToRemove.push(dpg);
...
function amonGenerateWanted(args)
{
	...
	args.metadata.eachTemplate(function iterMetadataEvent(pt) {
		...
		events.forEach(function (eventName) {
			...
			/*
			 * XXX it's a little dicey to fake up the probe group
			 * uuid like this.
			 */
		...
		jsprim.forEachKey(eventForSvc, function (svcname, eventName) {
			...
			zones = zonesBySvcName[svcname];
			zones.forEach(function (z) {
				pt.pt_checks.forEach(function (check, i) {
					wanted.addProbe({
					    'name': eventName + i,
					    'type': check.ptc_type,
					    'config': check.ptc_config,
					    'agent': z,
...
function amonUpdatePlanSummarize(plan)
{
