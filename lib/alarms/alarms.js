...
function amonLoadAlarmsForState(args, callback)
{
	...
	amon.listAlarms(account, options, function (err, rawalarms) {
		...
		/*
		 * XXX consider making this non-binary, so that we can report
		 * the alarms we did successfully parse, while still also
		 * printing warnings about the alarms that we couldn't.
		 */
		err = common.errorForList(errors);
		callback(err, err === null ? rv : null);
	});
}
