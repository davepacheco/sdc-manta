    ...
    function loadFromFile(filename, callback)
{
	...
	fs.readFile(filename, readoptions, function (err, contents) {
		...
		parsed.forEach(function (p, i) {
			self.loadFromObject({
			    'input': p,
			    'originLabel': filename + ': probe %d' + (i + 1)
			});
		});
...
/*
 * Loads a probe template from a parsed object.
 */
ProbeTemplates.prototype.loadFromObject = function loadFromObject(args)
{
	var inp;

	assert.object(args, 'args');
	assert.object(args.input, 'args.input');
	assert.string(args.originLabel, 'args.originLabel');

	/* XXX validate against JSON schema */
	inp = args.input;
	this.pts_probe_templates.push({
	    'pt_event': inp.event,
	    'pt_scope': {
	        'ptsc_service': inp.service
	    },
	    'pt_type': inp.type,
	    'pt_config': inp.config,
	    'pt_ka': {
	        'ka_title': inp.ka.title,
	        'ka_description': inp.ka.description,
	        'ka_severity': inp.ka.severity,
	        'ka_response': inp.ka.response,
	        'ka_impact': inp.ka.impact,
	        'ka_action': inp.ka.action
	    }
	});
};
